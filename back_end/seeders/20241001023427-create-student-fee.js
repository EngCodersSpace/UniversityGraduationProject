...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    for (const studentData of students) {
      ...
      const system = studentData.student_system; // System type (Paid or not)
      ...
      for (let levelIndex = 1; levelIndex <= currentLevelId; levelIndex++) {
        ...
        for (let termIndex = 1; termIndex <= 2; termIndex++) {
          ...
          if (system === 'Paid' || system === 'موازي') {
            // Paid students logic (can pay once or twice in each term)
            const numberOfInstallments = faker.datatype.number({ min: 1, max: 2 }); // Randomly decide 1 or 2 installments
            let installments = [];
            ...
            if (numberOfInstallments === 1) {
              ...
              // Two installments
              const firstInstallment = faker.datatype.number({ min: 100, max: 400 }); // Random first installment
