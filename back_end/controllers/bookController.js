...
exports.downloadFile = async (req, res) => {
  try {
    ...
    // Fetch file details from the database (book, lecture, etc.)
    ...
    // Start a background thread for download
    const worker = new Worker("./utils/downloadWorker.js", {
      workerData: { filePath, id }
    });

    worker.on("message", (message) => {
      console.log("Background download completed:", message);
    });
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    // Base folder where files will be uploaded
    const uploadFolderBase = path.join(__dirname, '../storage/library');
    const { category } = req.body;

    // Validate category
    const validCategories = ["Lectures", "ExamsForms", "References"];
    if (!validCategories.includes(category)) {
      return cb(new Error("Invalid category specified."), false);
    }

    // Construct the folder path dynamically based on the category
  ...
  filename: (req, file, cb) => {
    // Create a unique file name based on the timestamp and original file name
...
exports.uploadFile = async (req, res) => {
  try {
    // Validate necessary fields in the body
    const { category, title, subject_id, added_by } = req.body;
    if (!category || !title || !subject_id || !added_by) {
      return res.status(400).json({ message: "Category, title, subject_id, and added_by are required." });
    }

    // Call the multer upload middleware to handle the file upload
    upload(req, res, async (err) => {
      if (err) {
        return res.status(400).json({ message: err.message });
      }

      // The file is available as req.file now
      const { file } = req;
      if (!file) {
      ...
      // Generate file paths using getStoragePaths function
    //   const { filePath, displayImagePath } = getStoragePaths(category, Date.now(), title); // Assuming `Date.now()` as ID
