...
const checkPermission = (target, action) => async (req, res, next) => {
  try {
    if (!req.user || !req.user.permission) {
      return res
        .status(401)
        .json({ error: "Permission denied: User not authenticated" });
    }
    ...
    if (!userRole) {
      return res
        .status(403)
        .json({ error: "Permission denied: Role not found" });
    }
    ...
    if (!userPermissions.includes(action)) {
      return res
        .status(403)
        .json({
          error: `Permission denied: Missing [${target}, ${action}] permission`,
        });
    }
